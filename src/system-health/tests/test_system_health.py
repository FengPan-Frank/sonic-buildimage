...
def test_service_checker_k8s_containers(mock_config_db, mock_run, mock_docker_client):
    ...
    # Mock POD container (should also be skipped as namespace=sonic)
    ...
    # The raw K8s container names should also not appear
    assert mock_snmp_container.name not in running_containers
    assert mock_restapi_container.name not in running_containers

    # No K8s containers should be monitored for critical processes
    assert 'snmp' not in checker.container_critical_processes
    assert 'restapi' not in checker.container_critical_processes

    # restapi is in CONTAINER_K8S_WHITELIST, so it's excluded from expected containers
    # snmp is NOT in whitelist, so it's still expected (will trigger 'not running' alert)
...
def test_service_checker_mixed_containers(mock_config_db, mock_run, mock_docker_client):
    ...
    assert 'database' not in checker.container_critical_processes
